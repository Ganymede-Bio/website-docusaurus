...
The <div className="gray_button"><UserOutlined /> Sign In</div> button should be available once authentication is configured for your tenant, which is generally going to be a web address accessible via any web browser with the URL [https://<your_tenant\>.ganymede.bio](https://<your_tenant>.ganymede.bio).
...
Click on the <div className="purple_button"><NodeIndexOutlined /> **Flow Editor**</div> button in the sidebar.
...
Click on the <div className="purple_button">**Manage <SettingOutlined />**</div> button in the upper right-hand corner of the header bar and select <div className="black_button">Add New Flow</div> from the right sidebar.
...
Click on the <div className="purple_button_navbar">**New <PlusCircleOutlined />**</div> button in the header bar and search for the term "excel" to filter for the relevant <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Excel_Read**</div> *node*.  On the Flow diagram, select the <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Excel_Read**</div> *node* and connect its top attachment point to the bottom attachment point of the <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Flow_Inputs**</div> *node*.
...
Repeat this process with the <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Transform_Py**</div> *node*.  Upon doing so, the Flow Editor window should look as follows:

<img alt="Quickstart Flow Init" src="https://ganymede-bio.mo.cloudinary.net/apiServer/QuickstartFlow_20230110.png"/>

Node attributes with <div className="green_text">green labels</div> indicate validation attributes.  In this example, the <div className="green_text">excel</div> node attribute on the Excel_Read _node_ refers to the file extension that the *flow* looks for during runs.  Click on the edit button, and modify the <div className="green_text">excel</div> node attribute to be ".xlsx", so that only files with ".xlsx" as an extension would be let through.  

Click <div className="purple_button_navbar">**Save** <CaretRightOutlined className="white_icon" /></div> in the header to save this change.  After making this change, the _flow_ will look as follows:
...
By default, <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Excel_Read**</div> treats the first row in the input file as the header row, so no modification is necessary for ingesting the sample file.

The <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Transform_Py**</div> node needs to be modified to calculate absorbance difference.  To do so, open up the backing notebook for <div className="dark_gray_button"><PlusCircleOutlined className="tree_node_icon"/> **Transform_Py**</div> by clicking on <EditOutlined className="purple_button"/> in the upper-right hand corner of the _node_.  This opens up the Jupyter notebook corresponding to the node in a separate tab on the browser.
...
Edit the query_sql string to query for the output of the Excel_Read node (replacing <tenant\> in the query shown below with the appropriate tenant name).  Run the cell by clicking on the <div className="jupyter_button">▶️ Run</div> button in the header.
...
Deployment takes roughly 1-2 minutes.  Once the deployment is completed, return to the <div className="purple_button"><NodeIndexOutlined /> **Flow Editor**</div> view and click <div className="light_blue_button">**Browse**</div> on the Flow Inputs _node_.  Select local storage and the PlateReader.xlsx file downloaded at the beginning of this quickstart.

Click on <div className="purple_button"><CheckCircleOutlined /> **Flow Runs**</div> in the left sidebar to monitor the run.
...
Click on <div className="purple_button"><FileSearchOutlined /> **Data Explorer**</div> in the left sidebar and run the following query (with <tenant\> replaced) to observe the processed results.
...
The associated file ingested into the flow in this quickstart can be found in the <div className="purple_button"><FolderOutlined /> **Files**</div> tab of Ganymede.
