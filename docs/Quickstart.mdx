...
This exposes a modal for adding a new _flow_.  Populate the _flow_ name and description as shown below, then hit the "Create Flow" button.
...
<!-- TODO: Last clause should remain in docs until adding a node gives some feedback -->

Click on the downward-pointing carat in the corner of the <div className="button darkblue_button">**Excel_Read**</div> node to expand it to edit its attributes. 
...
Change the **excel** node attribute from "excel" to ".xlsx" so that only files with `.xlsx` as an extension will be processed by this node (and other file types, such as `.xlsm` will be blocked). 
<!-- TODO: how? -->
...
Click <div className="button darkblue_button_mui">**Save**<SaveOutlined className="white_icon" /></div> in the header to save this change. Now you have a one-node flow that is capable of ingesting excel files. Let's add a second node to process the data into some useful output.
...
Right now the <div className="button darkblue_button">**Transform_Py**</div> node is in its default state, so let's edit it to make it do something useful specific to our Plate Reader data. To do so, open up the backing notebook for <div className="button darkblue_button">**Transform_Py**</div> by clicking on command prompt icon (<EditOutlined className="button darkblue_button_icon"/>) in the upper-right hand corner of the _node_. This opens up the Jupyter notebook that corresponds to this node in a separate tab on the browser.

<!-- need to find command prompt icon -->
...
Edit the `query_sql` string to query for the output of the Excel_Read node.  
...
Run the notebook cell by clicking on the <div className="button jupyter_button">▶️ Run</div> button in the header.
...
This takes the difference between the two columns `run1` and `run2`.
...
Deployment takes roughly 1-2 minutes.  Once the deployment is completed, return to the <div className="button darkblue_button"><BranchesOutlined /> **Flow Editor**</div> view and click <div className="button darkblue_button_mui"><ExpandLessOutlined className="white_icon" />**Expand**</div> in the Run Queue bar at the bottom of the screen to expand the input form for running nodes.  Select local storage and the PlateReader.xlsx file downloaded at the beginning of this quickstart.

:::tip
The associated file ingested into the flow in this quickstart can be now found in the <div className="button darkblue_button"><FolderOutlined /> **Files**</div> tab of Ganymede.

<img alt="Files tab input files" src="https://ganymede-bio.mo.cloudinary.net/apiServer/QuickstartFiles_20230110.png"/>
:::


Click <div className="button run_queue_button">**Run** <PlayCircleOutlined /></div> on the Flow Inputs _node_ to kick off the _flow_. The running flow run will now be visible below.
...
To aid with observing data outputs from nodes, table heads can be shown by clicking on the <div className="button darkblue_button_mui">**View** <DashboardOutlined className="white_icon"/></div>
