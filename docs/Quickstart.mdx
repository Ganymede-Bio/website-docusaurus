...
This exposes a modal for adding a new _flow_. Populate the flow name and description as shown below, then hit the "Create Flow" button.
...
Click <div className="button darkblue_button_mui">**Save**<SaveOutlined className="white_icon" /></div> in the header to save this change. Now you have a one-node flow that is capable of ingesting Excel files. Let's add a second node to process this incoming raw data into some useful output.
