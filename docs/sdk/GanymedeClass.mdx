...
**GanymedeContext** stores [Flow](../FlowEditor.mdx) metadata such as user ID, input filenames, input parameters, and execution timestamp upon Flow execution.  The **GanymedeContext** object from the most recent run can be mimicked in testing code via the **Ganymede** object.
