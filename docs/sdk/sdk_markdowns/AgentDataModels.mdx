...
One example of how these functions can be used is to use `scan_for_finished_files` to scan a directory for files continuously, uploading them to Ganymede Cloud for processing when they are older than a specified number of minutes.  Such a flow would query previously uploaded files to Ganymede using the [list_files](../GanymedeClass.mdx#method-list-files) method to ensure that the same file is not uploaded multiple times.
