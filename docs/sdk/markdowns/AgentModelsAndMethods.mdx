...
### Accessing Ganymede tables from Agents

`agent.ganymede` contains convenience methods for accessing Ganymede cloud from Agents.

#### Functions

`read_sql_query` returns a Pandas DataFrame for a query executed against the Ganymede data lake.

- _param_ **sql_query**: str - SQL query string (in BigQuery format) to execute

```python
from agent.ganymede import read_sql_query

df = read_sql_query("SELECT * FROM plate_map WHERE experiment_id = 'exp123'")
```