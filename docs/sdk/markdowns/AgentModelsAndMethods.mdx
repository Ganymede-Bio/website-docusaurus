...
Objects for triggering a Flow from an [Agent](../../app/agents/Agent) can be found in `ganymede_sdk.agent.models`.  
...
This Agent would require _parent_dir_, _file_to_watch_, _recency_min_, and _recency_max_ to be configured.
...
def execute(**kwargs) -> UploadFileParams | None:
    ...
    watch_dir = labels.get('parent_dir', None)
    filename = labels.get('file_to_watch', None)
    recency_min = float(labels.get('recency_min', 0))
    recency_max = float(labels.get('recency_max', 3600))
