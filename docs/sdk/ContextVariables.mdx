...
- _param_ prefix_cols: Optional[list[str]]
    List of column prefixes to treat as a group.  For example, if the DataFrame has columns ['a_1', 'a_2', 'b_1', 'b_2'], then ['a_', 'b_'] would be the prefix_cols.  All examples that start with 'a_' or 'b_' would receive the same schema as the first element in the group.
- _param_ nullable_cols: Optional[list[str]]
    List of columns that are nullable in the schema. If not specified, all columns are considered nullable.
- _param_ required_cols: Optional[list[str]]
    List of columns that are required in the schema. If not specified, all columns are considered optional.

### Attributes
- _property_ df: pd.DataFrame
    Pandas DataFrame to validate
- _property_ schema_dict: dict[str, Column]
    Dictionary of column names and Pandera Column objects
- _property_ schema: DataFrameSchema
    Pandera DataFrameSchema object
- _property_ validated_df: pd.DataFrame
