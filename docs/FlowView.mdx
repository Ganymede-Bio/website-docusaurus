...
The <div className="button purple_button"><img src={useBaseUrl('/img/flows_icon.svg')} className="filter-white" /> **Flow View**</div> page provides access to running _flows_ and viewing run history.  This page can be accessed by clicking on a _flow_ on the <div className="button purple_button"><HomeOutlined /> **Home**</div> page.

## Running flows

To run a _flow_, drop a file into the upload box and click the <div className="button purple_button">Run <CaretRightOutlined /></div> button on the left side of the run box.
