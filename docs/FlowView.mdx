...
Flows can be run [from the browser](#manually-running-flows), from an [Agent](./connectivity/Agent), from [an event](./configuration/S3EventConfig), or by configuring a [browser-based file watcher](#watching-directories-for-new-files).  This sections below detail how Flows can be run from the browser.
