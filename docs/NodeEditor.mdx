...
- **Pencil Icon**: if present, the <EditOutlined className="button purple_button_icon"/> is a button that opens the user-editable notebook when clicked.
...
Editable nodes contain a pencil icon (<EditOutlined className="button purple_button_icon"/>) in their upper right hand corner that opens up the notebook backing the node upon button click.  This section walks through the code associated with such a node to discuss its functionality.
