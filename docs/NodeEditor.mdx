...
- **Terminal Icon**: if present, the <EditOutlined className="button darkblue_button"/> is a button that opens the user-editable notebook when clicked.
...
Editable nodes contain a terminal icon (<EditOutlined className="button darkblue_button"/>) in their upper right hand corner that opens up the notebook backing the node upon button click.
