...
On the Flow Editor page, nodes are rendered as a box with a header that contains the node name.  The node name can be edited by clicking the <div className="button dark_gray_button">Edit</div> button in the upper right hand corner of the node.  
...
For nodes with editable components, notebooks follow a common structure.  The sections of the notebook are described below.

#### Sections that are saved upon executing the Save Pipeline Code cell:
...
#### Sections that are discarded upon notebook close:
- **Save Pipeline Code** for saving and deploying user-defined SQL and Python pipeline code.  Upon save, linting is conducted using [ruff](https://github.com/astral-sh/ruff) on the code found in the **User-defined Python** cell.
- **Testing Section** for testing changes to user-defined SQL and Python
- **Linting Section** for linting user-defined SQL and Python, using [ruff](https://github.com/astral-sh/ruff)

The Testing and Linting Sections enable users to test changes to the user-defined SQL and Python prior to committing and deploying code.
...
Editable nodes contain a pencil icon (<EditOutlined className="button darkblue_button"/>) in their upper right hand corner that opens up the notebook backing the node upon button click.
These notebooks format code upon cell execution using [black](https://black.readthedocs.io/en/stable/), and perform type checking, hinting, and code diagnostics using [Jupyter LSP](https://github.com/jupyter-lsp/jupyterlab-lsp) configured with [PyRight](https://github.com/microsoft/pyright).  
...
<div style={{textAlign: 'center'}}>
  <img alt="Ganymede notebook tooltip"
      src="https://ganymede-bio.mo.cloudinary.net/apiServer/Notebook_Tooltip_20230906.png" />
</div>

#### Right click on a variable and select "Rename Symbol" to rename all instances of that variable:

<div style={{textAlign: 'center'}}>
  <img alt="Ganymede notebook rename symbol"
      src="https://ganymede-bio.mo.cloudinary.net/apiServer/Notebook_Rename_Symbol_20230906.png" />
</div>
...
Running this cell commits any code changes to the Git repository associated with the tenant and uploads the code to the workflow management system for executing code.  Upon save, linting is conducted.
