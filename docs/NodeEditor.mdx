...
#### Sections that are saved upon executing the Save Pipeline Code cell or typing `Cmd+Shift+S` (Mac) or `Ctrl+Shift+S` (Windows/Linux):
...
These cells have a light gray background.
...
- Typing `Cmd+Shift+S` (Mac) or `Ctrl+Shift+S` (Windows/Linux) to save the notebook.
- Selecting the _Kernel_ menu and then selecting _Save Commit and Deploy_

:::danger
Typing `Cmd+S` (Mac) or `Ctrl+S` (Windows/Linux) will not commit and deploy pipeline code, and so should not be used to save changes to pipeline code.
:::
...
A list of available _nodes_ and their associated categories can be found on the [table listing of node characteristics](./nodes/Overview.mdx#available-nodes-with-key-characteristics)  More detail can be found on the [Node documentation page](./nodes/Overview.mdx).
