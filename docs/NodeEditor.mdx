...
For nodes with editable components, notebooks that back nodes have the following structure:

- **User-defined SQL** for retrieving tabular data to be processed by the node
- **User-defined Python** for processing input
- **Save Pipeline Code** for saving and deploying user-defined SQL and Python pipeline code.  Upon save, linting is conducted.
- **Testing Section** for testing changes to user-defined SQL and Python
- **Linting Section** for linting user-defined SQL and Python, using ruff

The user-defined SQL and Python cells contain code that is executed by the workflow management system.  Executing the Save Pipeline Code cell commits the contents of these cells to a repo and deploys the contents of these cells to the workflow management system.  
...
from ganymede_sdk.editor import mock_ganymede_context
from ganymede_sdk.io import retrieve_sql

# Load jupyter_black, which formats cells using black when they are run
...
res = execute(input_file, ganymede_context=mock_ganymede_context)
res
```
