...
input_file = retrieve_files("sample_test.csv", mock_ganymede_context, input_or_output_bucket="input")

# Retrieves 2 files from the output bucket
output_files = retrieve_files(['sample_output.xlsx', 'sample_validation.txt'], mock_ganymede_context, input_or_output_bucket="output")
```
...
for f in list_files(mock_ganymede_context, input_or_output_bucket="input"):
  print(f)
````