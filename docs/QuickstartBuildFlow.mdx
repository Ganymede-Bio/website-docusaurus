...
:::tip Flows and Nodes
Flows are programs represented as directed graphs comprised of user-editable nodes. For precise definitions of Flows and nodes, check out [the Ganymede Concepts page](./Concepts.mdx).
:::
...
Click the <div className="button darkblue_button"><BranchesOutlined /> **Flow Editor**</div> button in the sidebar.

<img width="1124" alt="Quickstart" src="https://ganymede-bio.mo.cloudinary.net/apiServer/QuickstartSelectflowEditor_20230501.png"/>

### Step 3: Create a new Flow

Click the <div className="button darkblue_button">**Manage <SettingOutlined />**</div> button in the upper right-hand corner of the header bar and select <div className="button black_button">Add New Flow</div> from the right sidebar.
...
Now you have a one-node Flow that is capable of ingesting Excel files. Let's add a second node to process this incoming raw data.

Click the <div className="button darkblue_button">**Node** <PlusCircleOutlined /></div> button in the header bar again, and search for the term "Python". This time, add a <div className="button darkblue_button">**Python**</div> node to your Flow by clicking the <div className="button darkblue_button">**Add**</div> button.
...
This code calculates the difference between the two columns `run1` and `run2`, and stores the result in a new column called `run_diff`.

:::tip
Near the bottom of the notebook you'll find the **Testing Section**. As with the Input_File node, you can interactively test the SQL query and the **User-Defined Function**.
:::
...
If you encounter any challenges with this Quickstart, please visit the [Troubleshooting Flows page](./TroubleshootingDataProcessing) or [contact Ganymede Support](./Support.mdx).
