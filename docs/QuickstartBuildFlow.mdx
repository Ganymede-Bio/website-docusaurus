...
Save and deploy changes made to the Python node by clicking on the <SaveFilled className="button dark_gray_button"/>   button in the toolbar, or by pressing `Cmd+Shift+S` (Mac) or `Ctrl+Shift+S` (Windows/Linux).
