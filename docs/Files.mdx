...
### Deep links - generating URLs for files stored in Ganymede

URLs for files can be generated using the get_file_url function in the Ganymede SDK by calling the `get_file_url` method.
...
file_url = get_file_url(filename, bucket)
```

This can be useful for referencing files in Ganymede from other applications.
