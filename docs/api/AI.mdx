...
To set up access for any given environment, ensure that the following variables are set in [Environment Administration](../AdminControls):
...
The `code` function can be used to generate code snippets in editor notebooks, which can be used as the basis for the execute function.  This function generates code for the [User-Defined Function section of the notebook](../NodeEditor#backing-notebooks) based on a provided prompt and files provided. 
