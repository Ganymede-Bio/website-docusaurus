...
- [how to modify the agent code](./connectivity/Agent#watch-for-files-locally-then-run-flow) to add custom functionality.  Some examples of this are
  ...
  - delivering [multiple files into a single node](./api/markdowns/AgentDataModels#classes-associated-with-agent-triggered-flows)
- incorporating [agent file utilities](./api/markdowns/AgentDataModels#file-system-utilities) from the Ganymede SDK
