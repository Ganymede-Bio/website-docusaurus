...
Take care when configuring the watch folder path, since a configuration error here may be challenging to debug. Specifically:
* Ensure that forward slashes are used in the path specification.
* Ensure that the input path is specified correctly.  In the example shown, the Desktop is found within the OneDrive directory because OneDrive is being used to back up files stored on Desktop.  If OneDrive is not being used, the path would typically be be "C:/Users/\<username\>/Desktop/watch_folder".
...
Drop the [plate reader file](https://github.com/Ganymede-Bio/website-docusaurus/raw/main/PlateReader.xlsx) into the watch folder created in [step 3b](#step-3b-create-a-folder-to-watch).  The agent will detect the file and upload it to the Ganymede flow for processing.
...
If you run into any challenges with the Quickstart, please visit the [Troubleshooting Agents page](./connectivity/DebuggingAgents.mdx).
...
- [how to modify the agent code](./connectivity/Agent#watch-for-files-locally-then-run-flow) to add custom functionality.  Some examples of this are
  ...
  - delivering [multiple files into a single node](./sdk/sdk_markdowns/AgentDataModels#classes-associated-with-triggering-a-flow-from-an-agent)
