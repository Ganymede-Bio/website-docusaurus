...
#### Issue: All files are present in the directory watched.  In the logs within the app, files are shown as being collected in two different waiting groups, causing a failure upon flow trigger.  How do I ensure that all files appear in the same waiting group?
