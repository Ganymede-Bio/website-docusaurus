...
On Windows, ensure that the current system time is properly sync'd; if the time is not inaccurate, the Agent will show as disconnected.  To do so, right click on the time in the bottom right corner of the screen and select "Adjust date/time".  Ensure that the "Set time automatically" option is enabled, and click "Sync now".
