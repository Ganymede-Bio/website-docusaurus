...
Files can be tagged in user-defined code within flows and agents, but using slightly different methods.  Within flows, files are tagged by passing the path to the file into the `add_file_tag` function.  Within agents, files are tagged by passing the [FileParam](../sdk/sdk_markdowns/AgentDataModels#classes-associated-with-triggering-a-flow-from-an-agent) object into the `add_file_tag_to_fileparam` function.  The FileParam object holds the file that the Agent submits to Ganymede storage (for initiating a flow if the agent is configured to do so).
...
- _param_ **input_file_path**: str - The name of the file to tag.
...
```python
from ganymede_sdk.file_tag import add_file_tag, get_file_tags
...
To tag a file within an agent, use the `add_file_tag_to_fileparam` function in the `ganymede_sdk.agent` module.  This function takes the following arguments:
...
```python
from ganymede_sdk.agent import add_file_tag_to_fileparam
...
The example code below shows a Virtualization cleanup agent that tags all files transitting to Ganymede cloud storage with the Benchling Entry ID that was inputted to the session.
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> NoOpFileTagParams:
  ...
  for file in input_files:
    ...
    entry_tag = [tag for tag in tags if tag["tag_type_id"] == BENCHLING_ENTRY_TAG_TYPE_ID].pop()
