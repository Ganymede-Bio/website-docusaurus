...
- Agent Configuration Update: Agent configuration is updated through the agent page or the agent's processor notebook. This is a configuration that all connections for that agent adhere to. For example, if an agent that watches for files then runs a flow is updated to run a new flow, all compatible connections will begin to run that flow after they are updated. Every save to an agent notebook is considered a new agent version that will be retrieved and loaded by the agent.
- Connection Configuration Update: Updates made to the connection's configuration on its page will only update the running connection that it corresponds to. Other connections for that agent will not be affected. Connection-level changes include variables and labels.
...
There may be a disruption to any ongoing processing on the agent when an update is in progress so it is safest to request it when there is no activity on the instruments where the agents are running.
