...
Documentation on the objects used in the user-defined code for these agents can be found on the [Agent Data Models](../api/markdowns/AgentDataModels.mdx) page.
...
|------------|-------------|
| Status|  The current running status of the installed connection. |
| Agent | The agent from which the connection is running. |
| Connection Version | The version of the connection configuration currently running. Will be updated any time the connection configuration is updated. |
| Agent Version | The version of the agent configuration from which the connection is running. Will be updated any time the agent configuration or processor code changes. |
| Core Version | The version of the core agent infrastructure from which the connection is running. Will require a reinstall to update. |
| Strated At | The time the connection was first registered in Ganymede. |
| Last Status Ping | The last time a status ping was received from the connection by Ganymede. |
| PC Name | The hostname of the machine hosting the connection |
| MAC Address | The MAC address of the machine hosting the connection. |
| Machine IP  | The IP address Ganymede sees from the machine hosting the connection |
| Install Path | Directory path on the hosting machine the executable was installed. |
| Service Name | The Windows service name for the connection executable on the hosting machine. |
| Agent Tags | Tags that will be applied to all files uploaded by any connection running from the agent. These are changeable in agent configuration |
...
- Image: An image to associate with the connection
- Tags: The file tags to apply to files uploaded by the connection
- Variables: Key-value pairs that can be referenced in user-defined code
- Label: A list of strings that can be used to group connections in the Ganymede UI
