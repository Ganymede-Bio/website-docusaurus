...
To do so, follow the steps belwo:

1. **Setup Local User Accounts**:
...
3. **Share Folder**:
   - On the remote system, right-click the desired folder > `Properties` > `Sharing` > `Advanced Sharing`. Set permissions for the user.
4. **Map Drive**:
   - On the local system, use:
     ```
     net use Z: \\<remote-IP>\folder /user:<username> <password>
     ```
