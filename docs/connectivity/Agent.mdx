...
:::info
The input path can be specified on the ExecStart parameter in the systemd file.  If not specified, the agent 
will watch the same directory that the executable is found in.
:::
...
# to start the Agent service on system startup
...
```shell
# to reload the systemd daemon upon modifying the agent service file
sudo systemctl daemon-reload

# to view recent logs
journalctl -u ganymede_example_agent.service --no-pager | tail -100
