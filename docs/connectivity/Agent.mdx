...
- **StartLimitIntervalSec**: set to 0 to avoid service from stopping due to number of failures observed in a preset timeframe
- **Restart**: set to restart the Agent if it crashes
- **User**: user to associate Agent runs with
- **ExecStart**: path to the Agent executable.  -n specifies the name of the service.  -v specifies variables to pass to the Linux agent.
