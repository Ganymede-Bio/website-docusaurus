...
**Update Process**
A runtime update does not require any process restart or executable re-installation. As a result, the connection is able to accept changes to configuration while continuing its normal operations. The update is validated for integrity and the information is stored locally for access if it connection restarts for any reason in the future.
