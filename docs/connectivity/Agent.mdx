---
id: Agent
title: Configuring Agents and Connections
displayed_sidebar: webUiSidebar
---

import { DiffOutlined, PlusOutlined, SelectOutlined } from "@ant-design/icons";

Ganymede Connections are specialized programs that can run directly on an instrument computer in the lab. Each Connection consists of a combination of user-defined code and Ganymede configuration, providing flexibility in capturing files and interacting with the Ganymede ecosystem.
...
# Quick Start
After creating a Connection, users can build and install local executables called Connections, individual instances of a given Agent, on instrument PCs.
...
## Installing Connections corresponding to an Agent
...
Logs can be found on the Logs tab for each Agent and Connection.  More detail on these log messages can be found on the [Agent Logs page](./AgentLogs).
