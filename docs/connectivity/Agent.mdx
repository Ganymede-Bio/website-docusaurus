...
Documentation on the objects used in the user-defined code for these agents can be found on the [Agent Data Models](../sdk/sdk_markdowns/AgentDataModels.mdx) page.
...
You create custom logs that will be displayed in the Ganymede UI and local logs by using the `logger` kwarg in your user-defined processor code. For example:
