...
Select experimental runs from Ganymede's data lake, with automatic parsing of Ct values and metadata from various qPCR instruments.

#### 2. **Plate Layout Configuration**
Define sample positions, treatment groups, and controls. Use the lasso tool to exclude failed wells or outliers.

#### 3. **Normalization Setup**
Choose reference genes (housekeeping genes) and control conditions for ΔΔCt calculations.

#### 4. **Analysis Execution**
The app automatically calculates all derived metrics (ΔCt, ΔΔCt, fold change) based on your configuration.

#### 5. **Quality Review**
Assess replicate consistency, check melt curves, and validate using built-in quality metrics.

#### 6. **Results Export**
Generate reports, export data tables, and save publication-ready figures.
