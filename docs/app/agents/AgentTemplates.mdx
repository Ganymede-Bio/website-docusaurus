...
### Trigger Flow with indeterminate number of files when all files in watch directory are older than _stable_time_ seconds and no file is older than 1 day.

This example shows an Agent that captures a CSV file and an indeterminate number of Excel files from a directory.  The Agent will trigger a Flow if all files in the directory are older than [_stable_time_](./Agent#reserved-variables) seconds.
...
def get_param_mapping(
    ...
    match_dict["Read_CSV.csv"] = fp(watch_dir, parent_dir, "*csv")

    excel_files = glob.glob(os.path.join(watch_dir, parent_dir, "*.xlsx"))
    match_dict.update(dict(zip(excel_files, excel_files)))  # match each excel file uniquely
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TriggerFlowParams | None:
    ...
    excel_param = "Excel_Read_Multi.excel"
    input_path = kwargs.get("vars", {}).get("input_path", "")
    stable_time = float(kwargs.get("vars", {}).get("stable_time", 0))
    max_age = float(kwargs.get("vars", {}).get("max_age_time", 86400))  # 1 day by default

    if not input_path:
        raise GanymedeException(
            "input_path not specified in Agent variables.", exception_type="Validation"
        )

    # avoid sleeping every time the Agent is triggered
    for file_param in flow_params_fw.files.values():
        if isinstance(file_param, FileParam):
            file_upload_time = datetime.strptime(file_param.upload_ts, "%Y-%m-%dT%H:%M:%SZ")
            if (current_time - file_upload_time < timedelta(seconds=stable_time)) or (
                current_time - file_upload_time > timedelta(seconds=max_age)
            ):
                return None
        else:
            # handle list of files
            for f in file_param:
                file_upload_time = datetime.strptime(f.upload_ts, "%Y-%m-%dT%H:%M:%SZ")
                if (current_time - file_upload_time < timedelta(seconds=stable_time)) or (
                    current_time - file_upload_time > timedelta(seconds=max_age)
                ):
                    return None

    # ensure that enough time has elapsed since when the last file was captured
    time.sleep(stable_time)

    for file_param in flow_params_fw.files.values():
        if isinstance(file_param, FileParam):
            if current_time - os.path.getmtime(
                os.path.join(input_path, file_param.filename)
            ) < timedelta(seconds=stable_time):
                return None
        else:
            # handle list of files
            for f in file_param:
                if current_time - datetime.strptime(f.upload_ts, "%Y-%m-%dT%H:%M:%SZ") < timedelta(
                    seconds=stable_time
                ):
                    return None

    excel_file_param_list = []
    for file_param in list(flow_params_fw.files.values()):
        if isinstance(file_obj, list):
            for file_param in file_obj:
                if not file_param.filename.endswith(".xlsx"):
                    continue
                file_param.content_type = "application/vnd.ms-excel"
                file_param.param = excel_param
                excel_file_param_list.append(file_param)
        else:
            if not file_obj.filename.endswith(".xlsx"):
                continue
            file_obj.content_type = "application/vnd.ms-excel"
            file_obj.param = excel_param
            excel_file_param_list.append(file_obj)

    m = file_params_list_to_multi(excel_file_param_list)
    m.param = excel_param

    return TriggerFlowParams(
        single_file_params={"Read_CSV.csv": flow_params_fw.files["Read_CSV.csv"]},
        multi_file_params={excel_param: m},
        benchling_tag=None,
