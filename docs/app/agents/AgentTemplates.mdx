...
### Trigger Flow with indeterminate number of files when all files in watch directory are older than _stable_time_ seconds
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TriggerFlowParams | None:
    ...
    for file_param in flow_params_fw.files.values():
        if isinstance(file_param, FileParam):
            ...
            if current_time - file_upload_time < timedelta(seconds=stable_time):
                return None
        else:
