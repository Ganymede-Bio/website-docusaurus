...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TriggerFlowParams | None:
    ...
    # 
    # in this case, it's assumed that the files are located in the watch_dir; 
    # a common pattern is to repeat this logic based on parent_dir (not shown here for conciseness)
