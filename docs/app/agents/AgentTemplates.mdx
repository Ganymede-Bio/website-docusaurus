...
### Trigger Flow when all files in watch directory are older than _stable_time_ seconds
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TriggerFlowParams | None:
    ...
    if not input_path:
        raise GanymedeException("input_path not specified in Agent variables.", exception_type="Validation")
    ...
    for file_param in flow_params_fw.files.values():
        if isinstance(file_param, FileParam):
            if current_time - os.path.getmtime(os.path.join(input_path, file_param.filename)) < timedelta(seconds=stable_time):    
                return None
        else:
