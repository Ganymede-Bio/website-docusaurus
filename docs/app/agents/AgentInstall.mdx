...
### Step 4: Service Identity Setup (Optional, as needed)
Note: Currently this feature is supported from connection installed on Windows only
Introduced in core version 5.3.20, a section below describes a workaround for earlier versions
...
4. Click “Add Secret” and add both your username and password separately, remember the name of the secrets for each
:::note
  Important: The domain must be included as part of the username. If the username is created local as mentioned in “Setting Up Local Account” then “.\” should be prepended to the username to indicate it is a local account.
:::
...
:::note
  Important: Any changes to the secret values will propagate to the connection and force the service to restart with that information.
:::
