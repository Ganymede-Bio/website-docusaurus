...
After downloading the agent installer, modify permissions for the installer to 755 and run the installer.  The connection name is how the connection will be identified in the Ganymede UI.  
```bash
# make the agent installer executable
chmod 755 agent_name_<timestamp>_agent

./agent_name_<timestamp>_agent \<connection name\>
```


Running the agent installer will yield the following items in ~/\<connection name\>/:

- LinuxAgent_\<timestamp\>: The agent executable
- connection.yaml: The configuration file for the agent
- uninstall.sh: The uninstallation script

### Step 3: Configure the Agent Service

Create a [systemd service file](https://www.freedesktop.org/software/systemd/man/systemd.service.html) similar to this named _connection_name_.service_ in the same directory as the agent executable.  The service file should contain the following:
...
```bash
sudo cp /path/to/_connection_name_.service /etc/systemd/system/
sudo chmod 644 /etc/systemd/system/_connection_name_.service
```
