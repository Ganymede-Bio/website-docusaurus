...
In the example _flow_ shown above, the output of <NodeChip text="CSV_Write" /> uses the **flow_run_id** template variable (converted to ISO format) to add a timestamp to the CSV file output.

### Available template variables

- **flow_run_id**: Epoch time in ms when _flow_ was kicked off for manually triggered flows; guaranteed to be unique per _flow_ run
...
- **params**: Run metadata; dictionary containing flow initiator, and file inputs to the flow.
