...
**Improvements**
- **UI**: UI should be more responsive in Environments with lots of Flow Runs
- **Flows**: Missing or invalid columns will now cause a Flow Run to fail
- **Agents**: Agent logs will now include other running processes for better debugging

**Bug Fixes**
- **Agents**: Fixed an issue where Agent Service identity would not be updated correctly

**Agent Version**: 5.4.17

### February 20 2025

**Improvements**
- **UI**: File sorting by Flow now uses the Flow name instead of the Flow ID

**Bug Fixes**
- **Agents**: Fixed an issue where File detection could result in a File being detected twice

**Agent Version**: 5.4.10
...
**Improvements**
- **Notebook**: Notebook save confirmation is now more responsive, displaying as a purple box hovering over the header bar in notebooks.

**Bug Fixes**
- **UI**: Secrets can be created in Environment that do not have Flows enabled
- **UI/Agents**: Fixed issue where Connection variable/label configuration would not be saved consistently
- **Agents**: Fixed an issue where File detection could result in a File being detected twice
...
- **Analysis**: Analysis notebooks can be duplicated by typing `Alt`+`N` on Windows or `Option`+`N` on Macs within the notebook.
