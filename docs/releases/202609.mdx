...
- **API**: New public API endpoints for audit logs, environments, export credentials, secrets, and connection health are now available.
    ...
    - `getConnectionHealthRecords` / `getConnectionHealthRecord` - For Tempo-connected tenants, list Tempo connection health records for a given environment, or retrieve a record for a specific connection.
