...
**Features/Updates**
- **Notebooks**: A [code](../sdk/AI.mdx#code-generation) function has been added to development environment notebooks to allow users to generate code replicating the function signature of the pipeline cell.

**Improvements**
- **Flows**: [S3 read](../nodes/App/S3_Read.md) node now has a parameter for specifying behavior of duplicate files and a parameter for only syncing files with specific aging
...
**Bug Fixes**
- **SDK**: Fixed an issue in the [create_assay_results_from_dataframe](../sdk/markdowns/benchling.benchling#function-benchlingcreate_assay_results_from_dataframe) function where upload was failing on bulk upload.


### May 23 2024

**Features/Updates**
- A generic webhook is now available to support event-triggered flows from any service that supplies webhook events.

**Improvements**
- **Agent**: Agents of the same type built from the latest version can now run on the same PC without issues. (requires Agent to use v4+)
- **Agent**: Exceptions and errors in processor code are now properly logged in the Agent logs. (requires Agent to use v4+)
- **Notebooks**: The `help()` function is now [available for notebooks in development environments](../sdk/AI#search-documentation) available to query the documentation site.

**Bug Fixes**
- An issue where manual updates to data table schemas and descriptions were encountering errors has been resolved.

### May 16 2024

**Improvements**
- **Agent**: Agent exceptions in user-defined code are now logged; enabling this feature will require an Agent reinstall.
- **Notebooks**: Customers can now [customize the HTML template](../sdk/SendingAlerts#sending-emails-from-execute-function) when sending email alerts from user-defined code.
