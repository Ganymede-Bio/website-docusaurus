...
<img alt="Flow Editor header" src="https://ganymede-bio.mo.cloudinary.net/apiServer/FlowEditorHeader_20230310.png" />
&nbsp;
...
* **Rerun**: This requeues the last row that was ran for this specific flow. The button is greyed out if the current session had no runs (reloads reset this). If inputs have changed, this button will fill out all parameters that exist using the existing run.
* **Run All** : Runs all flows that are queued in the flow
...
<img width="324" alt="FCS_Parse" src="https://user-images.githubusercontent.com/46538575/225914733-31675cc7-aa5f-40e3-9b6f-a41edecbaa3b.png"/>
...
Input settings can be found on the top right of the input node:

<img width="550" alt="Sample input node with settings" src="https://storage.googleapis.com/ganymede-bio-website/public/apiServer/InputNodeWithSettings.png"/>

Clicking here will bring up the modal: 

<img width="550" alt="Sample input node with settings" src="https://storage.googleapis.com/ganymede-bio-website/public/apiServer/AutoRunModal.png"/>

#### AutoRuns
Inside input settings, we have the capability to automatically run whenever all inputs are filled. This button is disabled if there is a Multi-File input parameter or if there are no input parameters. Works with filewatcher and will work while not on the flow editor page.
