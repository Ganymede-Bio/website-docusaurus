...
Flows can be configured to automatically run on a schedule. The schedule is set by selecting from the dropdowns show below and can be configured in intervals of months, weeks, days, hours, and minutes. Note the schedule is set in UTC time.
