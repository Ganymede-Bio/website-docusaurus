...
<img alt="Flow Editor header" src="https://ganymede-bio.mo.cloudinary.net/apiServer/FlowEditorHeader_20240723.png" />

* **Choose a Flow**: Selection box for specifying the flow to load.
* **Node**: Opens modal for specifying new node to add.  
...
* **Manage**: Opens right sidebar to create new flows or delete existing ones. When a flow is selected, we can apply new configurations to the flow.
  ...
  * **Auto run trigger** - Scheduled times to run the flow automatically if enabled.
...
<img alt="Run queue bar" src="https://ganymede-bio.mo.cloudinary.net/apiServer/RunQueueBar_20230405.png" />
...
Clicking the <div className="button run_queue_button">**Run** <PlayCircleOutlined /></div> button on the Flow Inputs _node_ kicks off the flow run.  Run progress can be tracked by clicking on the **Runs** button in the header, or by navigating to the [<div className="button darkblue_button"><CheckCircleOutlined /> **Flow Runs**</div>](FlowRuns.mdx) page.
